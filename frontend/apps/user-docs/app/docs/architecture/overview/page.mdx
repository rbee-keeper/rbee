...
**Key features:**
- HTTP API server (default port: 8500)
...
```
1. Queen starts → Listens on port 8500
...
localhost:
  - queen (port 8500)
  - hive (connects to localhost:8500)
  - keeper (connects to localhost:8500)
...
gaming-pc.local:
  - hive (connects to server.local:8500)
...
- **Queen:** 8500 (HTTP API, configurable)
...
**Multi-machine setup:**
- Allow port 8500 (queen) from hive machines
