  ...
  parameters: {
    ...
    docs: {
      description: {
        component: `
## Overview
BeeArchitecture is a specialized diagram molecule that visualizes the rbee system architecture. It shows how queen-rbee orchestrates multiple PCs, where each PC has its own rbee-hive managing local devices.

## Composition
This molecule is composed of:
- **Queen node**: Orchestrator (brain) with crown emoji - can be on any PC
- **PC containers**: Each PC has its own rbee-hive (resource manager) with honeycomb emoji
- **Worker nodes**: GPU/CPU devices with bee emojis, managed by their PC's hive
- **Connectors**: Visual lines showing the hierarchy

## Architecture
The correct architecture is:
1. **queen-rbee** orchestrates everything (can run on any PC)
2. Each **PC** has its own **rbee-hive** (resource manager)
3. Each **rbee-hive** manages devices on that specific PC (GPUs, CPUs, etc.)
...
## Variants
- **Single PC**: One PC with its own hive managing local devices
- **Multi-host**: Multiple PCs, each with its own hive managing local devices
  ...
  render: () => (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-6">Scale across multiple machines for enterprise workloads.</p>
        <BeeArchitecture
          topology={{
            mode: 'multi-host',
            hosts: [
    ...
    <div className="w-full">
      <div className="mb-6 text-center">
        ...
        <p className="text-muted-foreground">
          Each PC runs its own rbee-hive managing local devices, orchestrated by queen-rbee
        </p>
      </div>
      <BeeArchitecture
        topology={{
